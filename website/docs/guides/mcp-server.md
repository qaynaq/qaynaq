...
Qaynaq's coordinator includes a built-in [Model Context Protocol](https://modelcontextprotocol.io/) (MCP) server at `/mcp`. Any flow with the [MCP Tool](/docs/components/inputs/mcp-tool) input becomes a tool that AI assistants can discover and call. Tools sync automatically every 30 seconds. To also expose tools from external MCP servers on the same `/mcp` endpoint, see [Remote MCP Servers](/docs/guides/mcp-remote-servers) and [Local MCP Servers](/docs/guides/mcp-local-servers).
...
```bash
claude mcp add qaynaq -- npx mcp-remote "http://localhost:8080/mcp?token=at_your_token_here"
```
